...
int main (int argc, char** argv) {
	...
	if (cflag) {
		// character graphics
		// Yeah, CHARACTER graphics.
		int wwidth = (mflag) ? 4 : 8; // wanted width
		if ((SpriteGif->SWidth % wwidth != 0) || (SpriteGif->SHeight % 8 != 0)) {
		...
		int wwidth = (mflag) ? 12 : 24; // wanted width
...
void usage() {
	// print usage message
	fputs("Usage: ./spriter [-r] [-m] [-c] [filename]\n", stderr);
	exit(1);
}
